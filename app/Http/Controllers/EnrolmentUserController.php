...
class EnrolmentUserController extends Controller
{
    public function index(Request $request)
    {
        ...
        $statusCounts = [
            'pending' => EnrolmentUser::where('status', 'pending')->count(),
            'processing' => EnrolmentUser::where('status', 'processing')->count(),
            'resolved' => EnrolmentUser::where('status', 'resolved')->count(),
            'rejected' => EnrolmentUser::where('status', 'rejected')->count(),
        ];

        return view('bvnuser', compact('enrollments', 'searchbvn', 'statusFilter', 'statusCounts'));
    }

    public function show($id)
    {
        $enrollmentInfo = EnrolmentUser::findOrFail($id);
        ...
        return view('bvnuser-show', compact('enrollmentInfo', 'statusHistory', 'user'));
    }
    ...
    public function update(Request $request, $id)
{
    ...
    try {
        $enrollment = EnrolmentUser::findOrFail($id);
        ...
        DB::commit();
        return redirect()->route('bvnuser.index')->with('successMessage', 'Status updated successfully.');
    } catch (\Exception $e) {
        DB::rollBack();
        return redirect()->route('bvnuser.index')->with('errorMessage', 'Failed to update status: ' . $e->getMessage());
    }
}
}
