<x-app-layout>
<x-slot name="title">Manual funding Control Form</x-slot>


<div class="container py-4">
    <div class="row justify-content-center">
        <div class="col-lg-7 col-md-9">
            <div class="card shadow">
                <div class="card-header bg-primary text-white d-flex align-items-center">
                    <i class="bi bi-cash-coin me-2 fs-4"></i>
                    <h5 class="mb-0">Manual funding Control Form</h5>
                </div>

                <div class="card-body">
                    {{-- Alart Message --}}
                    @include('modal.alart')
                  

                    {{-- Validation Errors --}}
                    @if ($errors->any())
                        <div class="alert alert-danger alert-dismissible fade show" role="alert">
                            <strong>Whoops!</strong> Please fix the following:
                            <ul class="mb-0">
                                @foreach ($errors->all() as $error)
                                    <li>{{ $error }}</li>
                                @endforeach
                            </ul>
                            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                        </div>
                    @endif

                    <form method="POST" action="{{ route('manual.funding') }}" id="fundingForm">
                        @csrf

                        <div class="mb-3">
                            <label for="email" class="form-label fw-semibold">User Email</label>
                            <input type="email" class="form-control" id="email" name="email"
                                value="{{ old('email') }}" required placeholder="Enter user's email">
                            <div class="form-text">Enter the user's registered email address</div>
                        </div>

                        <div id="user-info" class="mb-3" style="display: none;">
                            <h6 class="text-primary">User Information</h6>
                            <p class="mb-0">
                                <strong>Name:</strong> <span id="user-name"></span>
                            </p>
                        </div>

                        <div class="mb-3">
                            <label for="transaction_type" class="form-label fw-semibold">Transaction Type</label>
                            <select class="form-control" id="transaction_type" name="transaction_type" required>
                                <option value="">Select transaction type</option>
                                <option value="credit" {{ old('transaction_type') == 'credit' ? 'selected' : '' }}>Credit</option>
                                <option value="debit" {{ old('transaction_type') == 'debit' ? 'selected' : '' }}>Debit</option>
                            </select>
                        </div>

                        <div class="mb-3">
                            <label for="amount" class="form-label fw-semibold">Amount</label>
                            <div class="input-group">
                                <span class="input-group-text">₦</span>
                                <input type="number" class="form-control" id="amount" name="amount"
                                    value="{{ old('amount') }}" min="0.01" step="0.01" required placeholder="0.00">
                            </div>
                        </div>

                        <div class="mb-3">
                            <label for="description" class="form-label fw-semibold">Description</label>
                            <textarea class="form-control" id="description" name="description" rows="3"
                                placeholder="Enter transaction description">{{ old('description') }}</textarea>
                        </div>

                        <button type="submit" class="btn btn-primary w-100 fw-bold py-2">
                            <i class="bi bi-send me-2"></i> Process Transaction
                        </button>
                    </form>
                </div>
            </div>
        </div>
    </div>
</div>



<script>
document.addEventListener('DOMContentLoaded', function () {
    const emailInput = document.getElementById('email');
    const userInfo = document.getElementById('user-info');
    const userName = document.getElementById('user-name');

    let delayTimer;

    emailInput.addEventListener('input', function () {
        const email = emailInput.value;

        clearTimeout(delayTimer);

        delayTimer = setTimeout(function () {
            if (email.endsWith('.com')) {
                fetch(`/wallet/email-lookup?email=${encodeURIComponent(email)}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
                            userInfo.style.display = 'block';
                            userName.textContent = data.name;
                        } else {
                            userInfo.style.display = 'none';
                        }
                    })
                    .catch(() => {
                        userInfo.style.display = 'none';
                    });
            } else {
                userInfo.style.display = 'none';
            }
        }, 500);
    });
});
</script>

</x-app-layout>